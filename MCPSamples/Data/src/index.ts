...
const buildMatterUri = (clientId: string, id: string) =>
  `${MATTER_URI_PREFIX}${encodeURIComponent(clientId)}/${encodeURIComponent(id)}`;

const parseMatterUri = (uri: string) => {
  if (!uri.startsWith(MATTER_URI_PREFIX)) return null;
...
const QueryMattersSchema = z.object({
  text: z
    ...
    .optional()
    .describe(
      "Free text to search across EN_US name + note/description fields (optional). Example: 'Sky acquisition'"
    ),
    ...
    tools: [
      {
        name: "queryMatters",
        description:
          "Query matters using structured filters (open/closed, date ranges, client) plus optional free-text across EN_US name/notes/descriptions. Returns matching matter resource links.",
        inputSchema: zodToJsonSchema(QueryMattersSchema),
      },
      {
        name: "searchMatters",
        description:
          "Legacy: Search matters by Name_En_US only (LIKE). Returns matching matter resource links.",
...
const buildContainsQuery = (text?: string) => {
  const t = text?.trim();
  if (!t) return null;
...
const loadMatterSummaryLegacy = async (searchTerms: string, limit: number) => {
  ...
  return result.recordset as MatterSummary[];
};

const loadMattersByQuery = async (args: z.infer<typeof QueryMattersSchema>) => {
  const pool = await getPool();
...
const loadMatterById = async (clientId: string, id: string) => {
  const pool = await getPool();
  const result = await pool
    .request()
    ...
    const content: any[] = [
      {
        ...
        text: `Found ${records.length} matter(s) matching "${searchTerms}":\n\n${records
          .map(
            (record, index) =>
              `${index + 1}. ${record.Name_En_US ?? "Unnamed Matter"} (Client ${record.ClientId}, Matter ${record.Id})`
          )
          .join("\n")}`,
      },
    ];
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  if (!record) throw new Error(`Matter not found for resource: ${uri}`);
...
app
  .listen(PORT, () => {
    ...
    console.log(
      `${timestamp()} 🔎 queryMatters text mode: ${USE_FULLTEXT ? "FULLTEXT (CONTAINS)" : "LIKE fallback"}`
    );
  })
  .on("error", (error) => {
