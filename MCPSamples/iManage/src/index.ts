...
function stripDollarSchema<T>(schema: T): T {
...
function extractMcpKeyFromUri(uri: string): string | null {
  // Accept:
  // - https://... ?mcpKey=<key>
  // - imanage://<key>
...
const lines =
  results.length === 0
    ...
    : results.map((r, i) => {
        ...
        const href =
          r.iwl && String(r.iwl).startsWith("http")
            ? String(r.iwl)
            : `https://${IMANAGE_SERVER}/work/#/document/${encodeURIComponent(
                (r as any).customerId ?? "" // if you have it
              )}/${encodeURIComponent(IMANAGE_LIBRARY_ID)}/${encodeURIComponent(
                (r as any).docId ?? ""
              )}`;
