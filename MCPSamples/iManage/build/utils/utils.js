...
export function stripDollarSchema(schema) {
...
function ensureAzureOpenAiConfig() {
    if (!AZURE_OPENAI_ENDPOINT || !AZURE_OPENAI_API_KEY || !AZURE_OPENAI_DEPLOYMENT) {
        throw new Error("Missing Azure OpenAI configuration. Set AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_API_KEY, and AZURE_OPENAI_DEPLOYMENT.");
    }
}
async function azureOpenAiChat(messages) {
    ensureAzureOpenAiConfig();
    const endpoint = AZURE_OPENAI_ENDPOINT.replace(/\/$/, "");
    const url = `${endpoint}/openai/deployments/${encodeURIComponent(AZURE_OPENAI_DEPLOYMENT)}/chat/completions?api-version=${encodeURIComponent(AZURE_OPENAI_API_VERSION)}`;
...
export function extractMcpKeyFromUri(uri) {
    // Accept:
    // - https://... ?mcpKey=<key>
    // - imanage://<key>
...
export async function imanageSearch(query, client, matter, maxResults) {
...
export async function resolveIManageDocument(imanageUri) {
...
function chunkTextForLlm(text, maxChars, overlap) {
...
export async function summarizeDocumentText(text, maxChunkChars = 3000) {
    const chunks = chunkTextForLlm(text, maxChunkChars, Math.min(300, Math.floor(maxChunkChars / 10)));
    if (!chunks.length)
        return "The document has no extractable text to summarize.";
    const chunkSummaries = [];
    for (let i = 0; i < chunks.length; i++) {
        const summary = await azureOpenAiChat([
            {
                role: "system",
                content: "You are a legal document summarizer. Produce a concise, factual summary using only the provided chunk.",
            },
            {
                role: "user",
    ...
    return azureOpenAiChat([
        {
            role: "system",
            content: "You are a legal analyst. Consolidate chunk summaries into a final coherent document summary. Avoid repetition and keep only key facts.",
        },
        {
            role: "user",
...
export async function answerQuestionFromDocumentText(question, text, topChunks = 6, chunkSize = 2000) {
    const chunks = chunkTextForLlm(text, chunkSize, Math.min(250, Math.floor(chunkSize / 10)));
    if (!chunks.length)
        return "The document has no extractable text, so I cannot answer from it.";
    ...
    return azureOpenAiChat([
        {
            role: "system",
            content: "Answer the question only from the supplied context. If unsure, say what is missing. Cite chunk numbers like [Chunk 2].",
        },
        {
            role: "user",
